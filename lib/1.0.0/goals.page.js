...
(function(doc, locat) {
	...
	function GoalsView_reload() {
		this[0].innerHTML = '';
		var _private = get_private_by_public(this);
		_private.space = {};
		_private.goalsIds = {};
		_private.data = Goals.combine(_private.cacheData);
		this.data = {};
		init_pageview(this, _private);
	}
	...
	function page_jsonp(url, hash_info, cb) {
		...
		if(index > -1) {
			...
			req.jsonp(parse_url(url), common.PAGE_CALLBACK_NAME, function(data) {
				...
				data.onload();
				view_nodes(url, data.view, function(nodes) {
					cb(nodes, data, url);
				});
			});
		}
	...
	// 获取view所有节点
	function view_nodes(url, view, cb) {
		if(view) {
			if(view.charAt(0) === '/') {
				view_network_html(common.PROJECT_NAME + view + '.html', cb);
			}else if(view.startsWith('http')) {
				view_network_html(view, cb);
			}else if(view.startsWith('../')) {
				view_network_html(view + '.html', cb);
			}else {
				if(!Goals.xml) {
			...
			view_network_html(url + '.html', cb);
		}
	}
	
	// 获取网络view
	function view_network_html(url, cb) {
		req.ajax({
			url: url,
			resultType: 'xml',
	...
	// 开始解析data和view
	function parse_all(nodes, jsonp, url) {
		var page = init_page(nodes, jsonp);
		init_pageview(page._public, page._private);
		show_current_page(page);
	}
	 ...
	 */
	function init_pageview(_public, _privete) {
		var readynum = 0;
		function onready() {
			if(++readynum === 2) {
				_public.onready();
		...
		create_elems(_privete.nodes, _public, _privete);
		onready();
	}
...
//					bind_for_fn(r, parent);
		...
		function set_data_run(key, val, buffer) {
			if(Object.prototype.toString.call(val) === '[object Object]') {
				set_object_run(key, val, buffer[key]);
			}else if(Object.prototype.toString.call(val) === '[object Array]') {
				set_array_run(key, val, buffer[key]);
			}else {
				if(buffer[key] !== val) {
					buffer[key] = val;
		...
		function set_object_run(key, val, bufval) {
			...
			for(var k in val) {
				...
				if(Object.prototype.toString.call(r) === '[object Object]') {
					set_object_run(key + '.' + k, r, bufval[k]);
				}else if(Object.prototype.toString.call(r) === '[object Array]') {
					set_array_run(key + '.' + k, r, bufval[k]);
				}else {
					if(bufval[k] !== r) {
						bufval[k] = r;
		...
		function set_array_run(key, val, bufval) {
			for(var i = 0, r; i < val.length; i ++) {
				r = val[i];
				if(Object.prototype.toString.call(r) === '[object Object]') {
					set_object_run(key + '[' + i + ']', r, bufval[i]);
				}else if(Object.prototype.toString.call(r) === '[object Array]') {
					set_array_run(key + '[' + i + ']', r, bufval[i]);
				}else {
					if(bufval[i] !== r) {
						bufval[i] = r;
